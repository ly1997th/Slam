...
public:  
	...
	{
		 detect(image,harrisResponse);

	}
	void detect(const cv::Mat &image,Mat & harrisResponse)
	{  
		//opencv自带的角点响应函数计算函数  
		cv::cornerHarris (image,cornerStrength,neighbourhood,aperture,k);  
		harrisResponse=cornerStrength.clone();
		int threshold2=0;
		cv::threshold(cornerStrength,harrisResponse1,threshold2,255,cv::THRESH_BINARY); 
		// harrisResponse1=cornerStrength.clone();
		double minStrength;  
		//计算最大最小响应值  
	...
	{  
        ...
        thresh=cornerTh.clone();       
        // 转为8-bit图  
        cornerTh.convertTo(cornerMap,CV_8U);
        // 和局部最大值图与，剩下角点局部最大值图，即：完成非最大值抑制  
        cv::bitwise_and(cornerMap,localMax,cornerMap);  
        localmax=cornerMap.clone();
        return cornerMap;  
	}  
	...
	//用圈圈标记角点  
	void drawOnImage(cv::Mat &image,const std::vector<cv::Point> &points,cv::Scalar color= cv::Scalar(255,255,255),  
	        			int radius=0.5, int thickness=1) 
